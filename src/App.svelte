<script>
  ...
  function addTextField(text = "Невероятная надпись!") {
    const id = genID();
    fetchFont(currentFont);
    const object = {
      id,
...
<svelte:window
  ...
  on:drop|preventDefault={onUploadImage} />
<Tailwind />
<main style="color: gray;" class="flex flex-col items-center py-16 bg-gray-10 border-gray-10 min-h-screen">
  <div
    class="fixed z-10 top-0 left-0 right-0 h-12 flex justify-center items-center
    bg-gray-100 border-b border-gray-150">
    <input
      type="file"
      id="image"
      name="image"
  ...
  {:else}
    <div class="w-full flex-grow flex justify-center items-center">
      <span class=" font-bold text-3xl text-gray-500">Переместите что-то сюда</span>
    </div>
  {/if}
</main>
